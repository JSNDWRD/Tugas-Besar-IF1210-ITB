    ...
    while (fgets(baris, sizeof(baris), fObatMap)) {
        ...
        while (baris[i] != '\0' && baris[i] != '\n') {
            if (baris[i] == ',' || baris[i] == ';') {
                currentData[current] = '\0';
                switch (kolomData) {
                    case 0: obatId = atoi(currentData); break;
                    case 1: penyakitId = atoi(currentData); break;
                    case 2: urutan = atoi(currentData); break;
                }
                current = 0;
                kolomData++;
            } else {
        ...
        currentData[current] = '\0';
        if (kolomData == 2) urutan = atoi(currentData);
        ...
        if (idx == -1) {
            ...
            for (int k = 0; k < 100; k++) obatMap->buffer[idx].obatId[k] = -1;
